...
Although it's possible to run the chatbot on your local machine, the fastest way to get started is by connecting to our OpenShift cluster.

## Explore the UI app on our OpenShift cluster

### 1. Obtain credentials

Contact RCAccelerator team for credentails allowing you to access the chatbot UI.
...
- **Search Similarity Threshold** – Currently uses cosine similarity (cosine of the angle between two non-zero vectors in an inner product space). A threshold that is too low may yield irrelevant documents, while a threshold that is too high may exclude relevant ones.
